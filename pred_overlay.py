...
BUILDS_FILE = "predecessor_builds_v2.json"
CACHE_FOLDER = "app_cache" # Main folder for all cached data
...
MAX_BUILD_ITEMS = 6 # The maximum number of items in a build

# --- UI Colors & Styles ---
BG_COLOR = "#111827"  # Dark Gray-Blue
FG_COLOR = "#F9FAFB"  # Off-white
ACCENT_COLOR = "#3B82F6" # Blue
FRAME_COLOR = "#1F2937" # Lighter Gray-Blue
TRANSPARENT_COLOR = "#010101" # A near-black color to be made transparent
...
class DataHandler:
    ...
    def fetch_game_data(self):
        """
        Fetches heroes and items, using a local cache first if available.
        """
        api_heroes, api_items = None, None

        if os.path.exists(HEROES_JSON_FILE) and os.path.exists(ITEMS_JSON_FILE):
            print("Loading game data from local cache...")
            try:
                with open(HEROES_JSON_FILE, 'r', encoding='utf-8') as f:
                    api_heroes = json.load(f)
                with open(ITEMS_JSON_FILE, 'r', encoding='utf-8') as f:
                    api_items = json.load(f)
                print("Successfully loaded data from cache.")
            except json.JSONDecodeError:
                print("Cache file corrupted, will re-fetch from API.")
                api_heroes, api_items = None, None
        ...
        if not api_heroes or not api_items:
            ...
            try:
                ...
                heroes_res.raise_for_status()
                items_res.raise_for_status()
                
                api_heroes = heroes_res.json()
                api_items = items_res.json()

                with open(HEROES_JSON_FILE, 'w', encoding='utf-8') as f:
                    json.dump(api_heroes, f)
                with open(ITEMS_JSON_FILE, 'w', encoding='utf-8') as f:
                    json.dump(api_items, f)
                print("Game data fetched and saved to cache.")
            except requests.RequestException as e:
                print(f"FATAL ERROR: Could not fetch game data: {e}. App may not function correctly.")
                return

        self.heroes = sorted(
            [{'id': h['name'].lower(), 'name': h['display_name'], 'icon': h['image']} for h in api_heroes],
            key=lambda x: x['name']
        )
        ...
        # --- MODIFICATION: Added "Active" to the exclusion list ---
        EXCLUDED_SLOT_TYPES = ["Crest", "Trinket", "Consumable", "Potion", "Active"]
        EXCLUDED_RARITIES = ["Common", "Uncommon", "Rare"]
        
        items_with_class = []
        for i in all_items_full_data:
            if i.get('slot_type') not in EXCLUDED_SLOT_TYPES and i.get('rarity') not in EXCLUDED_RARITIES:
                item_data = {
                    'id': i['name'].lower(), 
                    'name': i['display_name'], 
                    'icon': i['image'],
                    'hero_class': i.get('hero_class'),
                    'rarity': i.get('rarity') 
                }
                items_with_class.append(item_data)

        self.items = items_with_class
        ...
        for crest in crests_full_data:
            requirements = crest.get('requirements', [])
            for req in requirements:
                if req in crest_map_by_internal_name:
                    if req not in self.crest_evolution_map:
                        self.crest_evolution_map[req] = []
                    child_data = {
                        'id': crest['name'].lower(), 
                        'name': crest['display_name'], 
                        'icon': crest['image'], 
                        'internal_name': crest['name']
                    }
                    self.crest_evolution_map[req].append(child_data)
                    all_children_internal_names.add(crest['name'])

        for crest in crests_full_data:
            if crest['name'] not in all_children_internal_names:
                self.base_crests.append({
                    'id': crest['name'].lower(), 
                    'name': crest['display_name'], 
                    'icon': crest['image'], 
                    'internal_name': crest['name']
    ...
    def get_image(self, path, size):
        ...
        if (path, size) in self.image_cache: return self.image_cache[(path, size)]
        ...
        if os.path.exists(local_path):
            try:
                ...
                return photo
            except Exception as e: print(f"Error opening cached image {local_path}: {e}")
        try:
    ...
    def get_placeholder(self, size):
        if ('placeholder', size) in self.image_cache: return self.image_cache[('placeholder', size)]
...
class App(tk.Tk):
    def __init__(self, data_handler):
        ...
        self.title("Predecessor Overlay")
    ...
    def display_expanded_view(self):
        ...
        self.hero_list_container.pack(side="left", fill="y", padx=5, pady=5)
        self.hero_list_container.pack_propagate(False)
    ...
    def draw_build_viewer(self):
        ...
        hero_icon = self.data.get_image(self.selected_hero['icon'], (80, 80))
        hero_label = tk.Label(header, image=hero_icon, bg=BG_COLOR); hero_label.image = hero_icon
        hero_label.pack(side="left", padx=10)
        ...
        build_list_scroll_area = ScrollableFrame(self.build_view_frame, bg=BG_COLOR)
        build_list_scroll_area.pack(fill="both", expand=True, padx=10)
        ...
        for lane in LANES:
            if builds_by_lane[lane]:
                ...
                for build in builds_by_lane[lane]:
                    ...
                    btn = tk.Button(display_frame, text="SELECT", bg=BG_COLOR, fg=FG_COLOR, bd=0, command=lambda b=build: self.select_active_build(b))
                    btn.pack(side="left", padx=(0, 5))
                    for item in build.get('items', []):
                        item_icon = self.data.get_image(item['icon'], (40, 40)); item_label = tk.Label(display_frame, image=item_icon, bg=FRAME_COLOR); item_label.image = item_icon
                        item_label.pack(side="left")
                    if build.get('crest'):
                        tk.Frame(display_frame, bg="gray", width=2, height=30).pack(side="left", padx=5)
                        crest_icon = self.data.get_image(build['crest']['icon'], (40, 40)); crest_label = tk.Label(display_frame, image=crest_icon, bg=FRAME_COLOR); crest_label.image = crest_icon
                        crest_label.pack(side="left")
    ...
    def draw_build_editor(self):
        ...
        editor_frame = tk.Frame(self.build_view_frame, bg=BG_COLOR)
        editor_frame.pack(fill="both", expand=True)

        top_frame = tk.Frame(editor_frame, bg=BG_COLOR)
        ...
        tk.Label(controls_frame, text="Name:", bg=BG_COLOR, fg=FG_COLOR).pack(side="left")
        name_var = tk.StringVar(value=build_data['name']); tk.Entry(controls_frame, textvariable=name_var).pack(side="left")
        tk.Label(controls_frame, text="Lane:", bg=BG_COLOR, fg=FG_COLOR).pack(side="left", padx=5)
        lane_var = tk.StringVar(value=build_data['lane']); ttk.Combobox(controls_frame, textvariable=lane_var, values=LANES, state="readonly").pack(side="left")
        current_build_frame = tk.Frame(top_frame, bg=BG_COLOR); current_build_frame.pack(pady=10)
        tk.Label(current_build_frame, text="Items", bg=BG_COLOR, fg=FG_COLOR).grid(row=0, column=0, pady=(0, 5))
        current_items_frame = tk.Frame(current_build_frame, bg=FRAME_COLOR, padx=5, pady=5); current_items_frame.grid(row=1, column=0)
        tk.Label(current_build_frame, text="Crest", bg=BG_COLOR, fg=FG_COLOR).grid(row=0, column=1, pady=(0, 5), padx=(10, 0))
        current_crest_frame = tk.Frame(current_build_frame, bg=FRAME_COLOR, padx=5, pady=5); current_crest_frame.grid(row=1, column=1, padx=(10, 0))
        
        action_frame = tk.Frame(editor_frame, bg=BG_COLOR)
        ...
        lists_container = tk.Frame(editor_frame, bg=BG_COLOR)
        ...
        def update_displays():
            ...
            for i, item in enumerate(build_data['items']):
                icon = self.data.get_image(item['icon'], (48, 48)); btn = tk.Button(current_items_frame, image=icon, bg=BG_COLOR, command=lambda idx=i: remove_item(idx)); btn.image = icon
                btn.pack(side="left")
            ...
            if build_data.get('crest'):
                icon = self.data.get_image(build_data['crest']['icon'], (48, 48)); btn = tk.Button(current_crest_frame, image=icon, bg=BG_COLOR, command=lambda: select_crest(None)); btn.image = icon
                btn.pack(side="left")
        ...
        item_scroll_container = ScrollableFrame(lists_container, bg=BG_COLOR)
        item_scroll_container.pack(fill="both", expand=True, pady=5)

        items_by_class = {}
        class_order = ['Marksman', 'Fighter', 'Tank', 'Mage', 'Support']
        
        for item in self.data.items:
            hero_class = item.get('hero_class')
            if hero_class and hero_class not in class_order:
                class_order.append(hero_class)
            
            if not hero_class:
                hero_class = "General"

            if hero_class not in items_by_class:
                items_by_class[hero_class] = []
            items_by_class[hero_class].append(item)

        if "General" in items_by_class:
            class_order.append("General")

        for hero_class in class_order:
            if hero_class in items_by_class:
                class_label = tk.Label(item_scroll_container.scrollable_frame, text=hero_class, font=("Segoe UI", 10, "bold"), bg=BG_COLOR, fg=ACCENT_COLOR)
                class_label.pack(anchor="w", padx=5, pady=(10, 2))

                class_frame = tk.Frame(item_scroll_container.scrollable_frame, bg=BG_COLOR)
                ...
                for i, item in enumerate(items_in_class):
                    ...
                    btn = tk.Button(border_frame, image=icon, bg=FRAME_COLOR, 
                                  bd=0, command=lambda it=item: add_item(it))
                    btn.image = icon
        ...
        tk.Label(lists_container, text="Crests", bg=BG_COLOR, fg=FG_COLOR).pack()
        crest_scroll_container = ScrollableFrame(lists_container, bg=BG_COLOR, height=250)
        crest_scroll_container.pack(fill="x", pady=5)
        
        tk.Label(crest_scroll_container.scrollable_frame, text="1. Select Base Crest", bg=BG_COLOR, fg=FG_COLOR).pack()
        base_crest_frame = tk.Frame(crest_scroll_container.scrollable_frame, bg=BG_COLOR)
        base_crest_frame.pack()
        
        tk.Label(crest_scroll_container.scrollable_frame, text="2. Select Evolution", bg=BG_COLOR, fg=FG_COLOR).pack()
        evolved_crest_frame = tk.Frame(crest_scroll_container.scrollable_frame, bg=BG_COLOR)
        evolved_crest_frame.pack(pady=2)

        tk.Label(crest_scroll_container.scrollable_frame, text="3. Select Legendary", bg=BG_COLOR, fg=FG_COLOR).pack()
        legendary_crest_frame = tk.Frame(crest_scroll_container.scrollable_frame, bg=BG_COLOR)
        legendary_crest_frame.pack(pady=2)
        
        base_crest_frame.image_refs = []
        evolved_crest_frame.image_refs = []
        legendary_crest_frame.image_refs = []
        ...
        def show_legendary_evolutions(stage1_crest):
            ...
            evolutions = self.data.crest_evolution_map.get(stage1_crest['internal_name'], [])
            for evo in evolutions:
                icon = self.data.get_image(evo['icon'], (48, 48))
                legendary_crest_frame.image_refs.append(icon)
                btn = tk.Button(legendary_crest_frame, image=icon, bg=FRAME_COLOR, command=lambda c=evo: select_crest(c))
        ...
        def show_stage1_evolution(base_crest):
            ...
            for w in legendary_crest_frame.winfo_children(): w.destroy()
            legendary_crest_frame.image_refs.clear()
            evolutions = self.data.crest_evolution_map.get(base_crest['internal_name'], [])
            for evo in evolutions:
                icon = self.data.get_image(evo['icon'], (48, 48))
                evolved_crest_frame.image_refs.append(icon)
                btn = tk.Button(evolved_crest_frame, image=icon, bg=FRAME_COLOR, command=lambda c=evo: show_legendary_evolutions(c))
                btn.pack(side="left", padx=2)

        for crest in self.data.base_crests:
            icon = self.data.get_image(crest['icon'], (48, 48))
            base_crest_frame.image_refs.append(icon)
            btn = tk.Button(base_crest_frame, image=icon, bg=BG_COLOR, command=lambda c=crest: show_stage1_evolution(c))
            btn.pack(side="left", padx=2)
    ...
    def save_build(self, build_data, name, lane):
        ...
        builds = self.all_builds[hero_id]
        found = False
    ...
    def select_active_build(self, build): self.active_build = build; self.display_collapsed_view()
...
if __name__ == "__main__":
    ...
    data = DataHandler()
    print("Game data processed successfully.")
    app = App(data)
    app.mainloop()